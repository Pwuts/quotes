import { Subquote } from '@prisma/client';
...
export function formatDate(date: Date | string): string
{
  ...
  if (nowT - dateT < dayDuration) {
    return now.getDay() == date.getDay() ? locale.strings.today : locale.strings.yesterday;
  }
    ...
    return getDateFormatter({ day: 'numeric', month: 'long', year: 'numeric' }).format(date);
  }
}
